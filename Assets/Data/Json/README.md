...
현재 파일들은 기존 TSV 자료를 새 JSON 구조로 옮긴 초기 변환본입니다.
아직 게임이 이 JSON을 직접 사용하지는 않으므로, 런타임 연결 작업과
회귀 검증이 끝나기 전까지 기존 TSV 파일을 삭제하면 안 됩니다.
...
현재 JSON은 `Tools/Convert-GameplayTsvToJson.ps1`을 사용해 기존 TSV에서
일괄 변환했습니다.
...
하지만 아직 다음 작업이 남아 있습니다.

1. 새 JSON 구조를 읽는 C# 데이터 클래스 작성
2. JSON 오류와 잘못된 참조를 검사하는 Unity 편집 도구 작성
3. JSON에서 런타임 데이터베이스 생성
4. 모든 몬스터 패턴과 판타지 효과 회귀 테스트
5. 기존 TSV 결과와 새 JSON 결과 비교
6. 검증 완료 후 TSV importer 제거

현재 게임은 아직 이 JSON을 직접 사용하지 않습니다. 런타임 전환이
완료되기 전에는 JSON만 수정해도 게임에 변경 사항이 반영되지 않습니다.

## 변환 도구 주의사항

마이그레이션 중 기존 TSV 자료로 JSON을 다시 만들 때만 프로젝트
루트에서 다음 명령을 사용할 수 있습니다.
...
기획자가 JSON을 새로운 원본으로 직접 수정하기 시작한 뒤에는 이
명령을 실행하면 안 됩니다. JSON에서 수정한 내용이 기존 TSV 자료로
덮어써질 수 있습니다.
